...
struct DragableObject: ViewModifier {
    ...
    init(
        onDragged: @escaping ((CGPoint) -> DragState),
        onDropped: @escaping ((CGPoint) -> Bool)
    ) {
    ...
    init(
        object: Dragable, onDragged: @escaping ((CGPoint) -> DragState),
        onDropObject: @escaping ((Dragable, CGPoint) -> Bool)
    ) {
    ...
    init(
        object: Dragable,
        onDragObject: @escaping ((Dragable, CGPoint) -> DragState),
        onDropped: @escaping ((CGPoint) -> Bool)
    ) {
        self.dragableObject = object
            ...
            .gesture(
                DragGesture(coordinateSpace: .global)
                    .onChanged { gesture in
                        ...
                        withAnimation(
                            .linear(
                                duration: DrawingConstants
                                    .dragStateOnChangedTransitionDuration)
                        ) {
                            self.dragState =
                                self.onDragged != nil
                                ? self.onDragged!(gesture.location)
                                : self.onDragObject!(
                                    dragableObject!, gesture.location)
                        }
                    ...
                    .onEnded { gesture in
                        successfulDrop =
                            self.onDropped != nil
                            ? self.onDropped!(gesture.location)
                            : self.onDropObject!(
                                dragableObject!, gesture.location)
                        withAnimation(
                            .linear(
                                duration: DrawingConstants
                                    .dragStateOnEndedTransitionDuration)
                        ) {
                            self.dragState = .none
                        }
...
extension View {
    ...
    public func dragable(
        ...
        modifier(
            DragableObject(
                object: object, onDragObject: onDragObject, onDropped: onDropped
            ))
    }
    ...
    public func dragable(
        ...
        modifier(
            DragableObject(
                object: object, onDragged: onDragged, onDropObject: onDropObject
            ))
    }
    ...
    public func dragable(
        ...
        modifier(
            DragableObject(
                object: object, onDragObject: onDragObject,
